...
def test_valid_bearer_get():
    ...
    response  = requests.get("http://localhost:9000/things/virtual-things-0/properties-jwt/", headers = headers)
    print(response.text)
...
def test_valid_bearer_put():
    ...
    data = {'on': False}
    response  = requests.put("http://localhost:9000/things/virtual-things-0/properties-jwt/", headers = headers, data = json.dumps(data))
    print(response.text)
    assert(response.status_code == 200)
