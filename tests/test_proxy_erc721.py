...
def server():
    ...
    p2 = subprocess.Popen(['python3', 'tests/http_server.py'])
...
def test_valid_bearer_get():
    ...
    response  = requests.get("http://localhost:9000/things/virtual-things-0/properties-erc721/", headers = headers)
    print(response.text)
...
def test_valid_bearer_put():
    ...
    data = {'on': False}
    response  = requests.put("http://localhost:9000/things/virtual-things-0/properties-erc721/", headers = headers, data = json.dumps(data))
    print(response.text)
    assert(response.status_code == 200)
