...
var authServerURL   = "blockchain.mmlab.edu.gr";
/*End configuration*/
...
var access = new web3.eth.Contract(ABI,contractAddress);
access.events.newDepositEvent().on('data', depositEventHandler);
...
function depositEventHandler(result)
{
    var lock = result.returnValues.lock
    console.log(`New deposit event for lock ${lock}`)
    var nonce = ''
    access.methods.getHashLock(lock).call().then(function(result){
        metadata = JSON.parse(result[0]);
        console.log(`Will request the key for ${metadata.nonce}`);
        var post_data = `{"nonce":"${metadata.nonce}"}`;
    
        var post_options = {
            host: authServerURL,
            port: '80',
            path: '/SOFIE/IAA/Authorization-Server/index.php?api=newDepositEvent',
            method: 'POST',
            headers: {
        ...
        var post_req = http.request(post_options, function(response) {
            ...
            response.on('end', function() {
                access.methods.openHashLock(lock, key).send({from:ethAccountAddr,gas: 3000000},function(error, result) { 
                    if (!error)
                    {
                        console.log(`Hashlock unlock invoked for lock ${lock} with key ${key}`);  
                    }else
                    {
                        console.log("Error: " + error); 
                    } 
